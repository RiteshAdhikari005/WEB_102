  ...
  try {
    ...
    if (req.user) {
      const like = await prisma.videoLike.findUnique({
        where: { userId_videoId: { userId: parseInt(req.user.id), videoId: parseInt(id) } }
      });
      video.isLiked = !!like;
    }
...
// Create video - accepts Supabase URLs from frontend
...
exports.updateVideo = async (req, res) => {
  try {
    ...
    if (!video) return res.status(404).json({ message: 'Video not found' });
    if (video.userId !== parseInt(userId)) return res.status(403).json({ message: 'Not authorized to update this video' });
...
exports.deleteVideo = async (req, res) => {
  try {
    ...
    // Delete files from Supabase Storage if paths exist
    if (video.videoStoragePath) {
      const supabase = require('../lib/supabase');
      await supabase.storage.from('videos').remove([video.videoStoragePath]);
    }
    if (video.thumbnailStoragePath) {
      const supabase = require('../lib/supabase');
      await supabase.storage.from('thumbnails').remove([video.thumbnailStoragePath]);
    }
